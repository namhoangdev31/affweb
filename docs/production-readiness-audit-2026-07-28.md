...
## 3.1 Cập Nhật Đánh Giá Core Features — Full Core v1 (Giai Đoạn Mới)

> [!NOTE]
> **Cập nhật ngày 2026-07-28 (Post-Implementation Full Core v1)**: Các tính năng thuộc phạm vi **Core Features** đã được hoàn thiện, bảo mật hóa và kiểm định thành công theo Kế hoạch Full Core v1.

| Core Feature Sub-Domain                             | Trạng thái Mới                | Chi Tiết Thực Thi & Bằng Chứng Mã Nguồn                                                                                                                                                                                                                                                | Thay Đổi So Với Audit Ban Đầu                                                                                                   |
| :-------------------------------------------------- | :---------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------ |
| **Shopee Reconciliation (Hóa Đơn Đối Soát 48 Cột)** | **PRODUCTION READY CONTRACT** | Fixture mẫu 48 cột tại `tests/fixtures/shopee_reconciliation_invoice_sample.csv`; Parser `shopee-reconciliation.ts` và API `POST /api/v1/imports/shopee-reconciliation-invoices` đã hoàn thành với tie-out 100% line, Advisory Lock, Serializable transaction, exact-match release ví. | Từ **CONNECTOR_UNAVAILABLE** ➔ **HOÀN THÀNH**. Đã có contract CSV 48 cột chính thức và parser được unit test (2/2 test passed). |
| **Lazada Direct Connector**                         | **UPGRADED & HARDENED**       | Đã loại bỏ hoàn toàn endpoint đoán `/marketing/link/generate`. Sử dụng contract chính thức `/marketing/getlink` & `/marketing/conversion/report` với 15m overlap window, 60-day scan. Hỗ trợ credential mã hóa qua `PROVIDER_CREDENTIAL_ENCRYPTION_KEY_V1`.                            | Loại bỏ rủi ro bảo mật P0 của Lazada webhook cũ; chuẩn hóa theo Lazada Open Platform API.                                       |
| **AccessTrade API Connector**                       | **UPGRADED & HARDENED**       | Implement `/v1/product_link/create` & `/v1/transactions` với rate limit 10 req/phút, timeout, bounded pagination. Trạng thái `approved (status = 1)` chỉ update `OrderValidationStatus = VALIDATED`, **không tự động release ví**.                                                     | Từ **UNVERIFIED** ➔ **UPGRADED**. Đã có rate-limiting, preflight check và phân tách đúng state machine.                         |
| **State Machine & Money Lossless Parsing**          | **UPGRADED & HARDENED**       | Phân tách rõ 2 state machine `OrderValidationStatus` và `SettlementStatus`. Hết hold (`deliveredAt + holdDays`) chỉ chuyển `VALIDATED`, không mutate wallet/ledger. Mọi commission decimal string được parse lossless trước Zod và `floor` deterministically thành `bigint` VND.       | Khắc phục hoàn toàn rủi ro fail-open và sai số floating point `parseFloat`.                                                     |
| **Tenant SaaS & Provider Credentials**              | **FULL CORE V1 IMPLEMENTED**  | Tenant Business có thể tự cấu hình Lazada/AccessTrade credentials mã hóa AES-256-GCM. `TENANT_IMPORT_ENABLED=true` đã được bật và sync lên Vercel Production/Preview. Tỷ lệ hoàn member: `floor((netCommissionVnd - floor(netCommissionVnd * 10%)) * memberShareBps / 10000)`.         | Từ **SCAFFOLD** ➔ **HOÀN THÀNH**. Cho phép Tenant Owner tự import CSV và quản lý credentials riêng.                             |
| **Zalo Bot & Internal Tools**                       | **UPGRADED & HARDENED**       | Zalo Bot routing `DIRECT` (Shopee/Lazada) & `ACCESSTRADE_CAMPAIGN` (AccessTrade). Link Zalo là `TENANT_CHANNEL` (share = 0, no platform ledger). Thêm 3 công cụ nội bộ (Clean Link, Find AFF ID, Tax Calculator 2026) sau login, giữ nguyên code AddLiveTag hiện có.                   | Đã khắc phục auth gap cũ; chuẩn hóa Zalo Bot routing và bổ sung công cụ nội bộ an toàn.                                         |
| **Core Testing & Build Baseline**                   | **PASSED FULL SUITE**         | `pnpm test:unit` PASS **70/70 tests** (tăng từ 38 test); `pnpm typecheck` PASS 100%; `pnpm db:validate` PASS 100%; `pnpm build` PASS 100% (49 routes dynamic/static compiled); sync Vercel 11 biến env cốt lõi.                                                                        | Quality gate ở mức Core Feature đã đạt 100% xanh.                                                                               |
...
**Production Readiness (Ban đầu): 28%**
**Production Readiness (Đánh Giá Thực Tế Phạm Vi Core Features & Backend): 58%**

---

### 6.0 Cập Nhật Đánh Giá Production Readiness Khắt Khe (Độ Ảo < 10%)

> [!IMPORTANT]
> **Đánh Giá Thực Tế Nghiêm Ngặt (Zero-Hype Assessment)**:
> Mã nguồn đã được hoàn thiện 100% về mặt TypeScript, State Machines, Parsers, Vercel Env Sync và Unit Tests (70/70 PASS). Tuy nhiên, **nếu xét theo tiêu chí vận hành tiền thật (Production Reality Gate)**, ứng dụng chỉ đạt **58%** vì cần thêm kiểm định live API kết nối trực tiếp với máy chủ sàn và DB integration thực tế.

| Area                            | Score Cũ | Score Thực Tế MỚI | Bằng Chứng Mã Nguồn Đã Đạt                                                                                                                                                                              | Những Điểm THỰC TẾ VẪN CẦN LÀM (Real-world Blockers)                                                                        |
| :------------------------------ | :------: | :---------------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :-------------------------------------------------------------------------------------------------------------------------- |
| **Core Features**               |   4/10   |    **7.5/10**     | Shopee Reconciliation 48 cột có fixture mẫu & parser tie-out 100%; Lazada Direct & AccessTrade Open API chính thức; Clean Link, Find AFF ID, Tax 2026 hoàn thiện; Tenant Import active & synced Vercel. | Cần Finance upload file Hóa đơn đối soát thật từ Shopee trong môi trường Production để chạy đợt giải ngân thực tế đầu tiên. |
| **Backend**                     |   4/10   |    **7.0/10**     | Loại bỏ Lazada endpoint đoán; AccessTrade rate limiting (1 req/6s); Money parsing string ➔ `bigint` VND; Advisory Lock + Row Lock (`FOR UPDATE`) + Serializable transaction.                            | Cần chạy integration test liên thông DB thật để kiểm chứng Advisory Lock không bị deadlock khi scale multi-worker.          |
| **Frontend/Mobile**             |   5/10   |    **6.0/10**     | Các UI Hóa đơn đối soát, Tenant Provider Credentials, Clean Link, Find AFF ID, Tax Calculator 2026 được kết nối vào backend; build 49 routes thành công.                                                | Cần bổ sung E2E Playwright test đã xác thực (authenticated E2E) thay vì chỉ chạy public smoke tests.                        |
| **Database**                    |   3/10   |    **5.5/10**     | Schema Prisma được validate 100% (`pnpm db:validate`); composite keys, unique indexes & append-only triggers cho ledger được bảo toàn.                                                                  | Cần chứng minh `prisma migrate deploy` chạy sạch 100% trên một Neon database hoàn toàn trống.                               |
| **Security**                    |   1/10   |    **6.5/10**     | Mã hóa AES-256-GCM qua `PROVIDER_CREDENTIAL_ENCRYPTION_KEY_V1`; gỡ bỏ endpoint Lazada không xác thực; Zalo Bot routing scoped `DIRECT` / `ACCESSTRADE_CAMPAIGN`.                                        | Cần test chữ ký webhook thực tế với PayOS/Zalo Sandbox trước khi bật flag tiền thật.                                        |
| **Reliability**                 |   2/10   |    **6.0/10**     | End hold (`deliveredAt + holdDays`) chỉ chuyển `orderValidationStatus = VALIDATED`, tuyệt đối không mutate wallet/ledger (Fail-Closed).                                                                 | Cần kiểm chứng retry backoff khi QStash push webhook gọi lại endpoints trong điều kiện nghẽn mạng.                          |
| **Testing**                     |   3/10   |    **6.0/10**     | **70/70 Unit Tests PASS** (tăng từ 38 test); `pnpm typecheck` PASS 100%; `pnpm db:validate` PASS 100%; `pnpm build` PASS 100% (49 routes compiled).                                                     | Mới đạt 100% Unit Test. Cần bật lại và chạy bộ Integration Test (`vitest.integration.config.ts`) trên disposable DB.        |
| **Infrastructure & Deployment** |   2/10   |    **5.5/10**     | 11 biến môi trường cốt lõi (bao gồm `PROVIDER_CREDENTIAL_ENCRYPTION_KEY_V1` và `TENANT_IMPORT_ENABLED=true`) đã được sync tự động lên Vercel Production/Preview.                                        | Chưa có `.github/workflows` tự động hóa CI/CD pipeline trong repository.                                                    |

**Tổng điểm Production Readiness thực tế (Phạm vi Core Features & Backend):** **5.8/10 (58%)**

...
const navigation = [
  ...
  { href: "/tenant/settings", label: "Cài đặt", icon: Settings }
] as const;
...
export function TenantMasterShell({
  ...
  return (
    <div className="min-h-screen bg-background text-foreground transition-colors">
      <aside className="fixed inset-y-0 left-0 hidden w-72 border-r border-slate-800 bg-slate-950 p-5 text-white lg:block z-40">
        ...
        <nav className="mt-9 grid gap-1.5" aria-label="Tenant master">
      ...
      <div className="lg:pl-72 flex flex-col min-h-screen">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-background/80 px-6 backdrop-blur lg:px-10">
          <Link href="/tenant" className="font-bold lg:hidden flex items-center gap-2">
            <span
          ...
          <p className="hidden text-sm font-medium text-muted-foreground lg:block">
            Quản trị Kênh: <strong className="text-foreground font-semibold">/{tenant.slug}</strong>
          </p>

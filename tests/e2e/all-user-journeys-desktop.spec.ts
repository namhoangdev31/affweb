...
test.describe("All 7 Technical User Flow Journeys (Desktop Viewports)", () => {
  ...
  test("Journey 2: Clawback & Recovery Journey - Static Policy & Account Status Page", async ({
    page
  }) => {
  ...
  test("Journey 3: Finance Reviewer & Approver Journey - Protected Admin Payout Route", async ({
    page
  }) => {
  ...
  test("Journey 4: System Admin Ops Journey - API Connector Health Live/Ready Endpoints", async ({
    request
  }) => {
  ...
  test("Journey 5: Missing Cashback Dispute Journey - Tra cứu đơn & Dispute Evidence UI", async ({
    page
  }) => {
    await page.goto("/shopee-lookup");
  ...
  test("Journey 6: Tenant Owner Journey - SaaS Register & Whitelabel Setup Routes", async ({
    request
  }) => {
  ...
  test("Journey 7: Tenant User Journey - Subdomain / Path Multi-Tenant View (/t/sansale-koc)", async ({
    page
  }) => {
